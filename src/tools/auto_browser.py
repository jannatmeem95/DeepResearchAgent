...
class AutoBrowserUseTool(AsyncTool):
    ...
    description = "A powerful browser automation tool that allows interaction with `wikipedia.org` web pages through various actions. Automatically browse the `wikipedia.org` and extract information based on a given task."
    parameters = {
        "type": "object",
        "properties": {
            "task": {
    ...
    async def _browser_task(self, task):
        ...
        assert self.model_id in ['gpt-4.1', 'langchain-Qwen'], \
            f"Model should be in [gpt-4.1, langchain-Qwen], but got {self.model_id}. Please check your config file."
